...
class SurveySession(models.Model):
    ...
    # Who opened the session, derived from the signed-in user's relation to the
    # survey and nothing else: the author checking their own survey must not
    # count as its first response. Existing rows default to `external`; the
    # history before this column is not rewritten.
    OPENED_BY_EXTERNAL = 'external'
    OPENED_BY_OWNER = 'owner'
    OPENED_BY_COLLABORATOR = 'collaborator'
    OPENED_BY_PREVIEW = 'preview'
    OPENED_BY_KINDS = (
        (OPENED_BY_EXTERNAL, 'external'), (OPENED_BY_OWNER, 'owner'),
        (OPENED_BY_COLLABORATOR, 'collaborator'), (OPENED_BY_PREVIEW, 'preview'),
    )
    opened_by_kind = models.CharField(max_length=12, choices=OPENED_BY_KINDS, default=OPENED_BY_EXTERNAL)
...
class SignupAttribution(models.Model):
    ...
    utm_medium = models.CharField(max_length=100, blank=True)
    utm_campaign = models.CharField(max_length=100, blank=True)
    # Path of the first marketing page seen (from the first-touch cookie); empty
    # for rows captured before the cookie existed.
