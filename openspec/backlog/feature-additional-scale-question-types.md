...
- **2026-08-15 — ranking shipped** in change `ranking-question`: a `ranking` input type where the respondent drags items into a strict order. Uniqueness is enforced rather than requested — the widget cannot express a tie and the server stores nothing unless the submission is a permutation of the items. Export gives one column per item holding its rank, which is what Jannis asked for. Still open in this item: continuous and vertical scales; follow-ups filed for a rank-aware analytics view and a "top N only" variant.
