# Backlog Index

**Regrouped 2026-08-31.** Ordered by epic, epics ordered by distance to revenue; within an epic by priority. The single priority that decides ordering: **start earning money** — a paid Pro tier a municipality or consultancy can put in a project budget ([epics/pro-tier.md](epics/pro-tier.md), memory `project_monetization_model`). Struck-through rows are shipped and kept for numbering.

Priority scale: **very high** = on the revenue path or a live buyer blocker · high = asked by a lead or protects the funnel · medium = parity / next quarter · low = someday.

**Next number: 171.**

## 1. Pro tier & monetization — the revenue path

Everything a buyer must see or sign before money can move, then the features buyers have actually asked for on calls. Sequence in [epics/pro-tier.md](epics/pro-tier.md#sequencing-2026-08-31).

| # | Type | Title | Priority | Area | Epic | Created |
|---|------|-------|----------|------|------|---------|
| 157 | feature | [Pro tier visible: pricing page, plan badge, copy rewrite](feature-pro-tier-visible-pricing-page.md) | **very high** | frontend | pro-tier | 2026-08-31 |
| 93 | feature | [Billing & invoicing for Pro (project-based)](feature-billing-invoicing.md) | **very high** | backend | pro-tier | 2026-07-29 |
| 88 | feature | [DPA / AVV compliance pack](feature-dpa-compliance-pack.md) | **very high** | general | pro-tier | 2026-07-29 |
| 87 | feature | [Workspace plans & entitlements (Free / Pro)](feature-workspace-plans-entitlements.md) | **very high** | backend | pro-tier | 2026-07-29 |
| 35 | feature | [Hosting geo-zone selection (EU / US)](feature-eu-data-hosting-option.md) | **very high** | infra | pro-tier | 2026-04-05 |
| 95 | feature | [AI-assisted response triage (relevance, spam, duplicates)](feature-ai-response-triage.md) | high | backend | pro-tier | 2026-07-31 |
| 90 | feature | [White-label branding](feature-white-label-branding.md) | high | frontend | pro-tier | 2026-07-29 |
| 89 | feature | [Custom domain for surveys and results pages](feature-custom-domain.md) | high | infra | pro-tier | 2026-07-29 |
| 11 | improvement | [Migrate servers to Frankfurt, Germany](improvement-frankfurt-server-migration.md) | high | infra | pro-tier | 2026-03-26 |
| 159 | feature | [AI translation of survey content](feature-ai-survey-translation.md) | medium | backend | pro-tier | 2026-08-31 |
| 158 | feature | [Project page: one public URL for the whole participation project](feature-project-page-hub.md) | medium | backend | pro-tier | 2026-08-31 |
| 94 | feature | [Grant reporting pack (Verwendungsnachweis support)](feature-grant-reporting-pack.md) | medium | backend | pro-tier | 2026-07-29 |
| 92 | feature | [AI analytics over survey responses](feature-ai-analytics.md) | medium | backend | pro-tier | 2026-07-29 |
| 42 | improvement | [Survey visual customization (fonts, colors)](improvement-survey-visual-customization.md) | medium | frontend | pro-tier | 2026-03-26 |

## 2. Team collaboration — the service we already deliver by hand

Us inside the client's workspace, and a consultancy with its municipality: discuss the survey where it lives, control who edits, hear about it by email. [epics/team-collaboration.md](epics/team-collaboration.md).

| # | Type | Title | Priority | Area | Epic | Created |
|---|------|-------|----------|------|------|---------|
| 169 | feature | [Comment threads anchored to survey objects](feature-survey-comment-threads.md) | high | backend | team-collaboration | 2026-09-15 |
| 91 | feature | [Workspace roles & permissions (read-only client access)](feature-workspace-roles-permissions.md) | high | backend | team-collaboration | 2026-07-29 |

## 3. Growth — more creators, more responses

Activation (47% of registrations never build a survey) and collection (distribution barely exists). Feeds the top of the paid funnel; nothing here bills.

| # | Type | Title | Priority | Area | Epic | Created |
|---|------|-------|----------|------|------|---------|
| 132 | feature | [Publish launch kit — ready-made distribution artifacts](feature-publish-launch-kit.md) | high | backend | growth | 2026-08-21 |
| 131 | feature | [Embeddable survey widget (iframe on the customer's own site)](feature-survey-embed-widget.md) | high | frontend | growth | 2026-08-21 |
| 130 | feature | [Auto-draft the public results page when a survey is published](feature-auto-draft-public-results-page.md) (promoted) | high | backend | growth | 2026-08-21 |
| 129 | feature | [AI audience plan — "how to reach your respondents" in the publish kit](feature-ai-audience-plan.md) | high | backend | growth | 2026-08-21 |
| 166 | idea | [LLM discoverability — being the answer when someone asks an AI](idea-llm-discoverability.md) | high | general | growth | 2026-09-03 |
| 81 | idea | [Interactive onboarding (guided first-survey wizard)](idea-interactive-onboarding.md) | high | frontend | growth | 2026-06-10 |
| 75 | feature | ["Made with Mapsurvey" viral loop on public pages](feature-made-with-mapsurvey-viral-loop.md) | high | frontend | growth | 2026-06-10 |
| 74 | idea | [Coursework / education channel — "Mapsurvey for classrooms"](idea-coursework-education-channel.md) | high | general | growth | 2026-06-10 |
| 168 | idea | [Conference abstract — Urban e-Planning 2027 (Track 3)](idea-conference-abstract-urban-eplanning-2027.md) | medium | general | growth | 2026-09-14 |
| 134 | improvement | [Test link is hidden in password settings while a Share button exists](improvement-test-link-in-share-dialog.md) | medium | frontend | growth | 2026-08-23 |
| 128 | improvement | [Share flow dead-ends: published-but-private surveys, no respondent path](improvement-share-flow-private-dead-end.md) | medium | frontend | growth | 2026-08-17 |
| 83 | feature | [Real-time updates for public results (live delivery layer)](feature-realtime-public-results.md) | medium | backend | growth | 2026-07-04 |
| 77 | idea | [Public results showcase gallery + SEO](idea-public-results-showcase-seo.md) | medium | general | growth | 2026-06-10 |
| 76 | feature | [Survey template gallery (onboarding / activation)](feature-survey-template-gallery.md) | medium | frontend | growth | 2026-06-10 |
| 78 | idea | [Localized growth in markets with existing traction](idea-localized-growth-markets.md) | low | general | growth | 2026-06-10 |

## 4. Community engagement — what incumbents have and we lack

The social layer inside the survey (`#152`, `#153`, `#160`) is what ThINK Jena / Ideenkarte-class buyers measure us on. See [Maptionnaire gap analysis](../../docs/marketing/competitors/maptionnaire.md) §3.

| # | Type | Title | Priority | Area | Epic | Created |
|---|------|-------|----------|------|------|---------|
| 152 | feature | [Ask questions about the creator's own map objects (rate, vote, comment)](feature-questions-on-overlay-features.md) | **very high** | backend | community-engagement | 2026-08-26 |
| 155 | feature | [Several geo questions on one map, each with its own colour and icon](feature-multi-geo-question-one-map.md) | high | frontend | community-engagement | 2026-08-26 |
| 153 | feature | [Show respondents the results so far, as a step inside the survey](feature-inline-results-step.md) | high | backend | community-engagement | 2026-08-26 |
| 170 | feature | [Feature-anchored answers — respond about a reference feature, not near it](feature-feature-anchored-answers.md) | high | fullstack | community-engagement | 2026-09-01 |
| 151 | feature | [Browse the creator's map objects — list, search, filter, click to zoom](feature-overlay-feature-browser.md) | high | frontend | community-engagement | 2026-08-26 |
| 160 | feature | [Reactions on other respondents' map markers (upvote / downvote)](feature-reactions-on-respondent-markers.md) | medium | backend | community-engagement | 2026-08-31 |
| 150 | improvement | [Editor guidance: attributes of a mapped object belong in sub-questions](improvement-subquestion-guidance-in-editor.md) | medium | frontend | community-engagement | 2026-08-25 |
| 149 | feature | [AI brief: "incident reporting / watch map" use case](feature-ai-brief-incident-reporting-use-case.md) | medium | backend | community-engagement | 2026-08-25 |
| 147 | feature | [Georeferenced image overlay — a plan drawing on top of the real map](feature-georeferenced-image-overlay.md) | medium | frontend | community-engagement | 2026-08-25 |
| 22 | feature | [QR code poster generator](feature-qr-code-poster-generator.md) | medium | frontend | community-engagement | 2026-04-02 |
| 21 | feature | [Live results projection mode](feature-live-results-projection.md) | medium | frontend | community-engagement | 2026-04-02 |
| 20 | feature | [Kiosk mode for event deployment](feature-kiosk-mode.md) | medium | frontend | community-engagement | 2026-04-02 |
| 19 | feature | [Map tagging with categorized pins](feature-map-tagging-pins.md) | medium | frontend | community-engagement | 2026-04-02 |
| 17 | feature | [Budget/token allocation question type](feature-budget-token-allocation.md) | medium | frontend | community-engagement | 2026-04-02 |
| 44 | feature | [Repeatable question groups](feature-repeatable-question-groups.md) | low | backend | community-engagement | 2026-04-02 |
| 43 | feature | [Narrative mapping (story + map point)](feature-narrative-mapping.md) | low | frontend | community-engagement | 2026-04-02 |

## 5. Survey analytics

Closest section to competitor parity; the cheap rows (word cloud, PNG, Excel) are demo-visible.

| # | Type | Title | Priority | Area | Epic | Created |
|---|------|-------|----------|------|------|---------|
| 156 | improvement | [Empty sessions clutter Responses — hide them and delete them in bulk](improvement-empty-session-cleanup.md) | high | frontend | survey-analytics | 2026-08-27 |
| 146 | improvement | [Reading the observations off the Responses map without GIS software](improvement-responses-map-read-observations.md) | high | frontend | survey-analytics | 2026-08-24 |
| 124 | feature | [AI activation analytics — hypothesis instrument](feature-ai-activation-analytics.md) | high | backend | survey-analytics | 2026-08-14 |
| 122 | improvement | [Survey page load performance](improvement-survey-page-performance.md) | high | frontend | survey-analytics | 2026-04-02 |
| 33 | feature | [Multi-color selection sets for analytics comparison](feature-analytics-multi-select-comparison.md) | medium | frontend | survey-analytics | 2026-04-02 |
| 32 | feature | [Analytics: hide/show and reorder questions](feature-analytics-question-visibility-order.md) | medium | frontend | survey-analytics | 2026-04-03 |
| 31 | feature | [A/B testing via survey versions (Phase 4)](feature-ab-testing.md) | medium | backend | survey-analytics | 2026-04-02 |
| 163 | feature | [Word cloud for open-text answers](feature-word-cloud.md) | low | frontend | survey-analytics | 2026-08-31 |
| 162 | feature | [Map image export (PNG) from Responses](feature-map-image-export.md) | low | frontend | survey-analytics | 2026-08-31 |

## 6. Field data collection

Olney-class volunteer campaigns. Pro for the collection-scale features; browser flow stays Free.

| # | Type | Title | Priority | Area | Epic | Created |
|---|------|-------|----------|------|------|---------|
| 154 | feature | [Bound where a respondent may draw — study area and exclusion zones](feature-drawing-bounds.md) | high | frontend | field-data-collection | 2026-08-26 |
| 148 | feature | [Draw a reference overlay layer directly in the editor](feature-draw-overlay-layer-in-editor.md) | medium | frontend | field-data-collection | 2026-08-25 |

## 7. Data management

Suite is shipped and Free; remaining items are long-tail.

| # | Type | Title | Priority | Area | Epic | Created |
|---|------|-------|----------|------|------|---------|
| 167 | improvement | [Rework response Warnings/Violations: hints, not errors](improvement-rework-response-warnings.md) | high | frontend | data-management | 2026-09-07 |
| 59 | feature | [Audit Trail (edit history log)](feature-audit-trail.md) | low | backend | data-management | 2026-04-05 |
| 58 | feature | [Inline Editing: geo answers (redraw polygon/point/line)](feature-inline-editing-geo.md) | low | frontend | data-management | 2026-04-05 |

## 8. Abuse prevention

Phase 1 shipped (Turnstile, rate limit, honeypot). Remaining items are hygiene.

| # | Type | Title | Priority | Area | Epic | Created |
|---|------|-------|----------|------|------|---------|
| 69 | feature | [Email Verification Before Account Creation](feature-email-verification-before-account.md) | high | backend | abuse-prevention | 2026-05-08 |
| 73 | feature | [IP / ASN Blocklist](feature-ip-asn-blocklist.md) | medium | backend | abuse-prevention | 2026-05-10 |
| 72 | improvement | [Signup Anomaly Dashboard](improvement-signup-anomaly-dashboard.md) | medium | backend | abuse-prevention | 2026-05-08 |
| 71 | feature | [Auto-Purge of Unconfirmed Signups](feature-unconfirmed-signup-purge.md) | medium | backend | abuse-prevention | 2026-05-08 |
| 70 | feature | [Disposable-Email-Domain Blocklist](feature-disposable-email-blocklist.md) | medium | backend | abuse-prevention | 2026-05-08 |

## 9. Bugs (no epic)

| # | Type | Title | Priority | Area | Epic | Created |
|---|------|-------|----------|------|------|---------|
| 133 | bug | [Leaked modal backdrops stack up and make the editor unclickable](bug-editor-modal-backdrop-leak.md) | high | frontend | — | 2026-08-17 |
| 126 | bug | [Survey name is silently truncated at 45 characters](bug-survey-name-silently-truncated.md) | high | backend | — | 2026-08-17 |
| 107 | bug | [Survey answers are never validated server-side](bug-answers-never-validated-server-side.md) | high | backend | — | 2026-08-05 |
| 105 | bug | [`SurveySession.end_datetime` is never written (0 of 4586 rows)](bug-session-end-datetime-never-set.md) | high | backend | — | 2026-08-05 |
| 2 | bug | [Image sub-question breaks point placement on geo question](bug-image-subquestion-breaks-geo-point.md) | high | frontend | — | 2026-04-25 |
| 125 | bug | [Http404 is reported to PostHog error tracking as if it were a defect](bug-http404-reported-as-error.md) | medium | infra | — | 2026-08-17 |
| 113 | bug | [Results-page preview always claims "Unlisted"](bug-results-preview-always-claims-unlisted.md) | medium | frontend | — | 2026-08-09 |
| 108 | bug | [Editing a question's type or choices invalidates stored answers](bug-question-edits-invalidate-stored-answers.md) | medium | backend | — | 2026-08-05 |
| 23 | bug | [Number field blank in CSV export](bug-number-field-blank-in-csv-export.md) | medium | backend | — | 2026-03-30 |

## 10. Unassigned features, improvements, ideas

| # | Type | Title | Priority | Area | Epic | Created |
|---|------|-------|----------|------|------|---------|
| 84 | feature | [Migrate transactional email to a dedicated ESP (deliverability)](feature-transactional-email-deliverability.md) | high | infra | — | 2026-07-05 |
| 79 | improvement | [Reduce geo-input friction (most-skipped question type)](improvement-reduce-geo-input-friction.md) | high | frontend | — | 2026-06-10 |
| 14 | feature | [Export filter: completed surveys only](feature-export-completed-only-filter.md) | high | backend | — | 2026-03-30 |
| 8 | feature | [Shapefile and GeoPackage export](feature-shapefile-geopackage-export.md) | high | backend | — | 2026-03-26 |
| 6 | feature | [Link questions to geoinformation after creation](feature-link-questions-to-geo.md) | high | frontend | — | 2026-03-26 |
| 165 | feature | ["Other, please specify" write-in option on choice questions](feature-other-option-with-write-in.md) | medium | frontend | — | 2026-09-02 |
| 161 | feature | [Excel (.xlsx) export with coordinates](feature-excel-export.md) | medium | backend | — | 2026-08-31 |
| 127 | improvement | [Language picker: `eu` (Basque) reads as "EU / European"](improvement-language-picker-eu-basque-trap.md) | medium | frontend | — | 2026-08-17 |
| 102 | feature | [Additional scale, matrix and ranking question types](feature-additional-scale-question-types.md) (ranking shipped) | medium | frontend | — | 2026-08-04 |
| 101 | feature | [Clear all responses / reset survey data](feature-clear-all-responses.md) | medium | backend | — | 2026-08-04 |
| 82 | feature | [Custom image basemap (non-geographic maps via CRS.Simple)](feature-custom-image-basemap.md) | medium | frontend | — | 2026-06-30 |
| 80 | improvement | [Reduce duplicate accounts / signup-login UX friction](improvement-account-dedup-signup-ux.md) | medium | backend | — | 2026-06-10 |
| 61 | idea | [Sub-question Discoverability Testing](idea-subquestion-discoverability-testing.md) | medium | general | — | 2026-04-14 |
| 37 | idea | [Digital Contour Maps — Geography Education](idea-digital-contour-maps-education.md) | medium | general | — | 2026-03-26 |
| 30 | feature | [Number field min/max validation](feature-number-field-min-max-validation.md) | medium | frontend | — | 2026-04-02 |
| 29 | feature | [Limit geopoint count per question](feature-limit-geopoint-count.md) | medium | frontend | — | 2026-03-30 |
| 25 | feature | [WMS/WFS basemap support](feature-wms-wfs-basemap.md) | medium | frontend | — | 2026-03-26 |
| 24 | feature | [Copy answer options between questions](feature-copy-answer-options.md) | medium | frontend | — | 2026-03-26 |
| 18 | feature | [Search in survey list](feature-search-in-survey-list.md) | medium | frontend | — | 2026-04-25 |
| 164 | feature | [Image as an answer option (choice / multichoice with pictures)](feature-image-answer-options.md) | low | frontend | — | 2026-08-31 |
| 40 | feature | [Answer choice templates](feature-answer-choice-templates.md) | low | frontend | — | 2026-03-26 |
| 39 | feature | [Progress bar for entire survey](feature-progress-bar.md) | low | frontend | — | 2026-03-26 |

## Shipped / closed

| # | Type | Title | Priority | Area | Epic | Created |
|---|------|-------|----------|------|------|---------|
| ~~103~~ | ~~improvement~~ | ~~[One geo question already accepts many features, and nobody knows it](improvement-multi-geometry-discoverability.md)~~ | ~~**very high**~~ | ~~frontend~~ | ~~—~~ | ~~2026-08-05~~ |
| ~~96~~ | ~~bug~~ | ~~[GeoJSON export: sub-question property inherits previous value](bug-geojson-subquestion-property-bleed.md)~~ | ~~**very high**~~ | ~~backend~~ | ~~—~~ | ~~2026-08-04~~ |
| ~~67~~ | ~~feature~~ | ~~[Registration Rate Limiting](feature-registration-rate-limiting.md)~~ | ~~**very high**~~ | ~~backend~~ | ~~abuse-prevention~~ | ~~2026-05-08~~ |
| ~~66~~ | ~~feature~~ | ~~[Registration CAPTCHA (Cloudflare Turnstile)](feature-registration-captcha.md)~~ | ~~**very high**~~ | ~~backend~~ | ~~abuse-prevention~~ | ~~2026-05-08~~ |
| ~~12~~ | ~~feature~~ | ~~[Conditional question visibility](feature-conditional-question-visibility.md)~~ | ~~**very high**~~ | ~~frontend~~ | ~~—~~ | ~~2026-03-30~~ |
| ~~1~~ | ~~feature~~ | ~~[GDPR / Security / Trust page](feature-gdpr-privacy-page.md)~~ | ~~**very high**~~ | ~~general~~ | ~~—~~ | ~~2026-03-26~~ |
| ~~123~~ | ~~bug~~ | ~~[Subtext is offered on every question type and rendered on four](bug-subtext-not-rendered-on-most-types.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-08-15~~ |
| ~~120~~ | ~~feature~~ | ~~[Survey event tracking (Phase 2)](feature-survey-event-tracking.md)~~ | ~~high~~ | ~~backend~~ | ~~survey-analytics~~ | ~~2026-04-02~~ |
| ~~119~~ | ~~feature~~ | ~~[Survey analytics dashboard (Phase 1 MVP)](feature-survey-analytics-dashboard.md)~~ | ~~high~~ | ~~frontend~~ | ~~survey-analytics~~ | ~~2026-04-02~~ |
| ~~117~~ | ~~feature~~ | ~~[Referrer tracking (Phase 2)](feature-referrer-tracking.md)~~ | ~~high~~ | ~~backend~~ | ~~survey-analytics~~ | ~~2026-04-02~~ |
| ~~114~~ | ~~bug~~ | ~~[Analytics and export disagree on which version they show](bug-analytics-and-export-disagree-on-version.md)~~ | ~~high~~ | ~~backend~~ | ~~—~~ | ~~2026-08-09~~ |
| ~~112~~ | ~~improvement~~ | ~~[Rework the question type picker — groups, icons, hints, example previews, bigger modal](improvement-group-question-type-list.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-08-09~~ |
| ~~111~~ | ~~bug~~ | ~~[Color, Icon and Image are offered on every question type](bug-question-fields-shown-for-every-type.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-08-09~~ |
| ~~110~~ | ~~improvement~~ | ~~[Publish prompt when sharing a Draft survey](improvement-publish-prompt-on-share-draft.md)~~ | ~~high~~ | ~~frontend~~ | ~~growth~~ | ~~2026-07-06~~ |
| ~~106~~ | ~~improvement~~ | ~~[Survey page always renders the map, squeezing non-geo surveys](improvement-map-panel-always-rendered.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-08-05~~ |
| ~~104~~ | ~~improvement~~ | ~~[Authors publish and close repeatedly because there is no way to try a survey](improvement-test-survey-without-publishing.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-08-05~~ |
| ~~100~~ | ~~improvement~~ | ~~[Closed survey is a dead end — no way back to editing](improvement-closed-survey-edit-dead-end.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-08-04~~ |
| ~~99~~ | ~~bug~~ | ~~[Range slider: endpoint labels not aligned to the track](bug-range-slider-label-alignment.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-08-04~~ |
| ~~98~~ | ~~bug~~ | ~~[Deleting a question silently destroys all answers](bug-editing-question-destroys-answers.md)~~ | ~~high~~ | ~~backend~~ | ~~—~~ | ~~2026-08-04~~ |
| ~~97~~ | ~~bug~~ | ~~[datetime answers never appear in the CSV export](bug-datetime-missing-from-csv-export.md)~~ | ~~high~~ | ~~backend~~ | ~~—~~ | ~~2026-08-04~~ |
| ~~86~~ | ~~bug~~ | ~~[Sub-question popup is too narrow (Leaflet default width)](bug-subquestion-popup-too-narrow.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-07-28~~ |
| ~~85~~ | ~~bug~~ | ~~[Rating question clips text labels](bug-rating-text-labels-clipped.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-07-28~~ |
| ~~68~~ | ~~feature~~ | ~~[Registration Honeypot Field](feature-registration-honeypot.md)~~ | ~~high~~ | ~~backend~~ | ~~abuse-prevention~~ | ~~2026-05-08~~ |
| ~~50~~ | ~~feature~~ | ~~Answer-level Linting: errors (geo bbox, self-intersection, required)~~ | ~~high~~ | ~~backend~~ | ~~data-management~~ | ~~2026-04-05~~ |
| ~~49~~ | ~~feature~~ | ~~Auto-Validation: basic rules (empty, incomplete, missing required)~~ | ~~high~~ | ~~backend~~ | ~~data-management~~ | ~~2026-04-05~~ |
| ~~48~~ | ~~feature~~ | ~~Bulk Operations toolbar~~ | ~~high~~ | ~~frontend~~ | ~~data-management~~ | ~~2026-04-05~~ |
| ~~47~~ | ~~feature~~ | ~~Clean Export (respects validation status)~~ | ~~high~~ | ~~backend~~ | ~~data-management~~ | ~~2026-04-05~~ |
| ~~46~~ | ~~feature~~ | ~~Session Validation Status (4 statuses + trash)~~ | ~~high~~ | ~~backend~~ | ~~data-management~~ | ~~2026-04-05~~ |
| ~~45~~ | ~~feature~~ | ~~Attribute Table in Analytics~~ | ~~high~~ | ~~frontend~~ | ~~data-management~~ | ~~2026-04-05~~ |
| ~~27~~ | ~~feature~~ | ~~[Public results map](feature-public-results-map.md)~~ | ~~high~~ | ~~frontend~~ | ~~pro-tier~~ | ~~2026-03-26~~ |
| ~~16~~ | ~~feature~~ | ~~[Funnel monitoring: source → registration → survey → responses](feature-funnel-monitoring.md)~~ | ~~high~~ | ~~backend~~ | ~~—~~ | ~~2026-04-25~~ |
| ~~15~~ | ~~idea~~ | ~~[AI agent that creates surveys from chat description](idea-ai-survey-creator-chat-agent.md) (promoted)~~ | ~~high~~ | ~~frontend~~ | ~~growth~~ | ~~2026-04-25~~ |
| ~~13~~ | ~~improvement~~ | ~~[Versioning: cross-version analytics and response counts](improvement-versioning-cross-version-analytics.md)~~ | ~~high~~ | ~~backend~~ | ~~—~~ | ~~2026-04-16~~ |
| ~~10~~ | ~~feature~~ | ~~[Show response count in editor dashboard](feature-response-count-in-editor.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-03-26~~ |
| ~~9~~ | ~~bug~~ | ~~[Pinned point disappears in subsequent sections](bug-pinned-point-disappears.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-03-26~~ |
| ~~7~~ | ~~feature~~ | ~~[Translate UI buttons and instructions](feature-ui-i18n.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-03-26~~ |
| ~~5~~ | ~~feature~~ | ~~[Range slider: from-to labels](feature-range-slider-labels.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-03-26~~ |
| ~~4~~ | ~~bug~~ | ~~[GeoJSON export not importable in QGIS](bug-geojson-qgis-import.md)~~ | ~~high~~ | ~~backend~~ | ~~—~~ | ~~2026-03-26~~ |
| ~~3~~ | ~~bug~~ | ~~[Export vs Download confusion](bug-export-download-confusion.md)~~ | ~~high~~ | ~~frontend~~ | ~~—~~ | ~~2026-03-26~~ |
| ~~121~~ | ~~feature~~ | ~~[UTM parameters & link generator (Phase 3)](feature-utm-link-generator.md)~~ | ~~medium~~ | ~~frontend~~ | ~~survey-analytics~~ | ~~2026-04-02~~ |
| ~~118~~ | ~~feature~~ | ~~[Results dashboard with map visualization](feature-results-dashboard.md)~~ | ~~medium~~ | ~~frontend~~ | ~~—~~ | ~~2026-03-26~~ |
| ~~116~~ | ~~feature~~ | ~~[Interactive Analytics Map](feature-interactive-analytics-map.md)~~ | ~~medium~~ | ~~frontend~~ | ~~—~~ | ~~2026-04-02~~ |
| ~~115~~ | ~~feature~~ | ~~[Cross-filtering / cross-highlighting in analytics dashboard](feature-analytics-cross-filtering.md)~~ | ~~medium~~ | ~~frontend~~ | ~~survey-analytics~~ | ~~2026-04-02~~ |
| ~~109~~ | ~~bug~~ | ~~[Multi-line template comments render into the page](bug-template-comments-leak-into-html.md)~~ | ~~medium~~ | ~~frontend~~ | ~~—~~ | ~~2026-08-05~~ |
| ~~62~~ | ~~bug~~ | ~~[Default map in survey settings cannot be None](bug-default-map-cannot-be-none.md)~~ | ~~medium~~ | ~~frontend~~ | ~~—~~ | ~~2026-04-25~~ |
| ~~60~~ | ~~improvement~~ | ~~[Hide language picker when single language](improvement-hide-language-picker-single-lang.md)~~ | ~~medium~~ | ~~frontend~~ | ~~—~~ | ~~2026-04-14~~ |
| ~~57~~ | ~~feature~~ | ~~Validation Settings (configurable thresholds per survey/question)~~ | ~~medium~~ | ~~frontend~~ | ~~data-management~~ | ~~2026-04-05~~ |
| ~~56~~ | ~~feature~~ | ~~Answer-level Linting: warnings (statistical outliers, suspicious text)~~ | ~~medium~~ | ~~backend~~ | ~~data-management~~ | ~~2026-04-05~~ |
| ~~55~~ | ~~feature~~ | ~~Auto-Validation: advanced rules (fast, geo outliers, duplicates, area)~~ | ~~medium~~ | ~~backend~~ | ~~data-management~~ | ~~2026-04-05~~ |
| ~~54~~ | ~~feature~~ | ~~Session Tags & Notes~~ | ~~medium~~ | ~~backend~~ | ~~data-management~~ | ~~2026-04-05~~ |
| ~~53~~ | ~~feature~~ | ~~Inline Editing: text, choice, number answers~~ | ~~medium~~ | ~~frontend~~ | ~~data-management~~ | ~~2026-04-05~~ |
| ~~52~~ | ~~feature~~ | ~~IDE-style Panel Layout (Map, Table, Charts, Anomalies)~~ | ~~medium~~ | ~~frontend~~ | ~~data-management~~ | ~~2026-04-05~~ |
| ~~51~~ | ~~feature~~ | ~~Anomalies Panel with checkboxes~~ | ~~medium~~ | ~~frontend~~ | ~~data-management~~ | ~~2026-04-05~~ |
| ~~41~~ | ~~feature~~ | ~~[Media upload question type (image, audio, video)](feature-audio-upload.md)~~ | ~~medium~~ | ~~backend~~ | ~~—~~ | ~~2026-03-26~~ |
| ~~38~~ | ~~feature~~ | ~~[Geolocation Map Centering](feature-geolocation-map-centering.md)~~ | ~~medium~~ | ~~frontend~~ | ~~—~~ | ~~2026-04-01~~ |
| ~~36~~ | ~~feature~~ | ~~[Feedback Widget](feature-feedback-widget.md)~~ | ~~medium~~ | ~~general~~ | ~~—~~ | ~~2026-03-30~~ |
| ~~34~~ | ~~improvement~~ | ~~[Finish Drawing Buttons for Polygon and Line](improvement-finish-drawing-buttons.md)~~ | ~~medium~~ | ~~frontend~~ | ~~—~~ | ~~2026-04-05~~ |
| ~~28~~ | ~~improvement~~ | ~~[Disable choices on number-type questions](improvement-disable-choices-on-number-questions.md)~~ | ~~medium~~ | ~~frontend~~ | ~~—~~ | ~~2026-03-30~~ |
| ~~26~~ | ~~feature~~ | ~~[Satellite and topographic basemap options](feature-satellite-basemap.md)~~ | ~~medium~~ | ~~frontend~~ | ~~—~~ | ~~2026-03-26~~ |
