...
and display blocks that collect nothing — where each type carries an icon, its display label and a
one-line hint of what the respondent does. Hovering a type SHALL show a canned example of that
type as a respondent would see it.
