...
## Purpose
Pull the acquisition metrics that the creator funnel's top depends on -- Google Search Console
impressions and clicks, Plausible landing traffic and channels -- into local daily records via a
scheduled, idempotent command, and record demo opens with their authentication state. Covers
credential handling, the backfill window, per-source failure isolation, and what "not configured"
means, so that a source in an unknown state is never rendered as a zero.
