...
## 8. Build "Survey settings" panel adopts the Publish page-settings look

- [x] 8.1 Promote the generic Publish page-settings CSS to shared classes in `editor_base.html` — `.pr-ctxbar` (+ `.ico`/`.t`/`.s`/`.btn`), `.pr-scroll`, `.pr-card` (+ `h6`), `.pr-field` (+ `label`), `.pr-help`, `.pr-switch` (+ `input`, `:last-child` no-border), `.pr-url`, `.pr-banner`; remove the now-duplicated definitions from `public_results.html` (only layout-specific `.pr-container`/`.pr-block .icon`/`.pr-main`/`.pr-preview*`/`.pr-slug-msg` stay there)
- [x] 8.2 Add `.pr-panel-flush { margin: -1.5rem; }` so the panel, swapped into the padded `.editor-main`, lets the sticky `.pr-ctxbar` span the full center column exactly like the Publish page
- [x] 8.3 Rewrite `survey_settings_panel.html` to the Publish structure: a `.pr-ctxbar` header (gear `ico` + "Survey settings" title + "Applies to the whole survey" subtitle) over a `.pr-scroll`, with each section a `.pr-card` (General / Default map position / Collaborators / Password & test access), form fields as `.pr-field` + `.pr-help` hints, and the geolocation toggle as a `.pr-switch`; all existing behaviour preserved verbatim (autosave `surveySettingsGeneralForm`, basemap-checkbox sync into hidden `id_basemaps`, Leaflet map picker + Save map position, HTMX collaborators, password modal include)
- [x] 8.4 Verify in-browser (`?panel=settings`) that ctxbar, cards, fields, switch, Leaflet picker, collaborators and password sections all render in the shared style; full `./run_tests.sh survey` green
