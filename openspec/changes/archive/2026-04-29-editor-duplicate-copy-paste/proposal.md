...
- Add a **Copy / Paste** flow using a `localStorage`-backed clipboard (key `editor_clipboard`). The buffer holds references `{kind, source_survey_uuid, source_id, label, input_type, copied_at}` — paste fetches the source fresh from the server. The clipboard is shared across browser tabs of the same origin, so a user can copy in one tab and paste in another tab editing a different survey. `input_type` is cached only for client-side gating of the "Paste as sub-question" button.
...
- **Sub-question semantics**: pasting a sub-question directly into a section promotes it to a top-level question. Pasting a regular non-geo question under a `point`/`line`/`polygon` parent attaches it as a sub-question (the source's own sub-questions are dropped in this case). Pasting a **geo-type** question (point/line/polygon) as a sub-question is REJECTED — this enforces the rule introduced by issue #17 that sub-questions cannot themselves be geo. The "Paste as sub-question" button is hidden when the clipboard contains a geo source.
