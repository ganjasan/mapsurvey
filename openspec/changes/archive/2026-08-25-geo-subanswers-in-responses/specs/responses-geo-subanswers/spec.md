...
On the Responses map, a pointer-mode click on a geo feature SHALL open a popup at the feature showing the question name and the feature's attribute rows, without changing the existing selection behavior of the click. Details-mode click behavior (opening the session modal) SHALL be unchanged. Popup content SHALL be inserted as text (never markup built from answer values).
...
- **THEN** the session is selected as before AND a popup opens listing each attribute as "name: value"
