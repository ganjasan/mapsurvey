...
- `PublicResultsBlock`: `page` (FK), `question` (FK, null for text/counter blocks), `block_type` (`chart`|`map`|`text`|`counter`), `viz` (`auto`|`bar`|`pie`|`heatmap`|…), `custom_title` (JSON, optional), `geo_label_fields` (JSON list), `order` (int).
