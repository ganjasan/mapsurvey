...
**Mobile-adaptive layouts (three kill switches)**: `MOBILE_BOTTOM_SHEET` turns the ≤768px
respondent question panel into a three-detent bottom sheet over an always-visible map
(`survey/assets/css/bottom-sheet.css` + `js/components/bottom_sheet.js`; the global
`toggleInfo` is wrapped so drawing tools snap the sheet to `peek` instead of hiding it) and
adds the "✓ N added" geometry-confirmation chip. `MOBILE_EDITOR_NAV` gives the editor
...
`css/editor-mobile.css` + `js/editor_mobile_nav.js`; everything double-gated by the
`mobile-nav-enabled` body class AND the media query, so desktop is untouched).
`EDITOR_AUTOSAVE` replaces Save/Apply on question EDIT forms with debounced autosave + a
loud saved/saving/error indicator on ALL viewports (the one intentional desktop change;
autosave POSTs carry `autosave=1` and validation errors return 422 JSON so the typed-in
form is never re-rendered). New-question forms keep an explicit Create button. All three
flags default off; unset = pre-change layout, which is the rollback story. Rollout order:
bottom sheet → editor nav → autosave. Touch reorder uses SortableJS `delay:300 +
delayOnTouchOnly` (long-press) — do not add ▲▼ reorder buttons, that was explicitly
rejected in review. Leaflet.draw tooltips pick tap-phrased strings via `pointer: coarse`
